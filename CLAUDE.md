...
- **Uploads** a file to Cloudflare R2 and prints the public download URL as a terminal QR code, or
- **Serves** a file over the LAN via an ephemeral HTTP server with a QR code (no R2 needed).

Entry point is `src/index.ts` (shebang `#!/usr/bin/env bun`), exposed as the `artifactuploadcli` bin.

## Commands

```bash
bun src/index.ts <file>            # upload to R2 + QR (alias: bun run start)
...
- `env.ts` — `loadEnvConfig()` loads `.env` from **cwd** (not the install dir) and merges CLI overrides. Exits the process with a missing-vars report if R2 config is incomplete. Upload path only.
- `upload.ts` — `uploadToR2()` (multipart via `@aws-sdk/lib-storage` `Upload`, streams from disk, prints `%` progress) and `detectContentType()` (extension→MIME map, used by both upload and serve).
...
Data flow (upload): `index` → `loadEnvConfig` → `uploadToR2` → `printResult`.
Data flow (serve): `index` → `serveFile` → `printServeInfo` → park process on signals.

## Critical gotchas

- **R2 + AWS SDK checksums**: the S3 client sets `requestChecksumCalculation`/`responseChecksumValidation` to `"WHEN_REQUIRED"`. R2 rejects the flexible checksum headers `@aws-sdk` v3.500+ sends by default — do not remove these.
...
`R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET` (or `--bucket`), `R2_PUBLIC_URL` (or `--public-url`). Live in `.env` (gitignored).
